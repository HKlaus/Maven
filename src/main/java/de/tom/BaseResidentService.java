...
import de.tom.ResidentRepository;
...
public class BaseResidentService implements ResidentService {
	...
	public Resident getUniqueResident(Resident filterResident)
		...
		if (residents.size() == 1) {
			...
			throw new ResidentServiceException(
					"Suchanfrage lieferte kein eindeutiges Ergebnis!");
		}
	}
	...
	public List<Resident> getFilteredResidentsList(Resident filterResident) {
		...
		Pattern givenNamePattern = compileFilterPattern(filterResident
				.getGivenName());
		Pattern familyNamePattern = compileFilterPattern(filterResident
				.getFamilyName());
		...
		for (Resident resident : residentRepository.getResidents()) {
			if (matchesFilter(resident, givenNamePattern, familyNamePattern,
					streetPattern, calendarFilter)) {
	...
	private boolean containsWildcards(Resident data) {
		if ((data.getGivenName() != null)
				&& (data.getGivenName().indexOf("*") != -1)) {
			return true;
		}
		if ((data.getFamilyName() != null)
				&& (data.getFamilyName().indexOf("*") != -1)) {
			return true;
		}
	...
	private Pattern compileFilterPattern(String attribute) {
		if ((attribute == null) || (attribute.trim().length() == 0)) {
			...
			return Pattern.compile(attribute.trim().toLowerCase()
					.replaceAll("\\*", ".*?"));
		}
	}

	private boolean matchesFilter(Resident current, Pattern givenNamePattern,
			Pattern familyNamePattern, Pattern streetPattern,
		...
		if (calendarFilter != null) {
			...
			if (calendarFilter.get(Calendar.YEAR) != calendarCurrent
					.get(Calendar.YEAR)) {
				return false;
			}
			if (calendarFilter.get(Calendar.MONTH) != calendarCurrent
					.get(Calendar.MONTH)) {
				return false;
			}
			if (calendarFilter.get(Calendar.DAY_OF_MONTH) != calendarCurrent
	...
	private boolean matchesAttribute(String originalCurrent, Pattern pattern) {
		String current = (originalCurrent == null ? null : originalCurrent
				.trim().toLowerCase());
		if (pattern == null) {
			return true;
		}
