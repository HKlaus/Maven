...
public class ResidentRepositoryMockTest {
	...
	@Test(expected = ResidentServiceException.class)
	public void uniqueResidentWildcards2Test() throws ResidentServiceException {
		// service.setResidentRepository(rep);
		service.getUniqueResident(new Resident("Name1", "*", null, null, null));
	}

	@Test(expected = ResidentServiceException.class)
	public void uniqueResidentWildcards3Test() throws ResidentServiceException {
		// service.setResidentRepository(rep);
	...
	public void getFilteredResidentsWildcardTest() {
		int size = service.getFilteredResidentsList(
				new Resident("N*", "F*", "S*", "S*", null)).size();
		assertEquals(size, 4);
		verify(rep);
	}
